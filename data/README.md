...
Per the [Challenge Project Overview](../Challenge-Project-Overview.md), the team's initial clause-detection work is scoped to the **10 most impactful clause categories**, with the remaining categories (up to the full 41) as a stretch goal.
...
`*.csv`, `*.json`, and `full_contract_txt/` under `data/` are excluded from version control via `.gitignore`. Never commit raw contract files to this repository.
